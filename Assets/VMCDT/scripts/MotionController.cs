...
public class MotionController : MonoBehaviour
{
	...
	public void SetPose(float[,] pose) {
		...
		for (int i = jointConfig.joints.Length - 1; i >= 0; --i) {
            ...
            //Vector3 aa = new Vector3(pose[i, 2], pose[i, 1], -pose[i, 0]);  // to unity coordinate
			...
			//jointConfig.joints[i].rotation = Quaternion.AngleAxis(Mathf.Rad2Deg * aa.magnitude, aa.normalized);
		}
	...
	public void SetRootPosition(float[] position) {
		...
		// Vector3 p = new Vector3(0, position[1], 0);  // to unity coordinate
		SetRootPosition(p);
	}
	...
	public void SetRootPosition(Vector3 rotates) {
		Vector3 position = new Vector3(0f, 1.9f, 0f);
		jointConfig.TranslationJoint.localPosition = position;
        jointConfig.TranslationJoint.localRotation = Quaternion.Euler(rotates[0], rotates[1], rotates[2]);
    }
